...
class ArucoCenterFixedOri(Node):
    def __init__(self):
        ...
        self.get_logger().info("Task3 node started — centroid TF only")
    ...
    def _process_fruits(self, frame):
        ...
        # ---------------- STRICT GREEN MASK ----------------
        # tighter green range to avoid sunlight-shifted false positives
        ...
        for c in cnts:
            ...
            # We inspect the lower body region of the bounding box which shows the fruit body.
            # If a sizable fraction of the body has H in purple range, reject.
            # ----------------------------------------------------
            ...
            if body_region is not None and body_region.size > 0:
                try:
                    ...
                    # Purple / violet hue in OpenCV HSV approx 130..170 (tune if necessary)
                    purple_mask = (H > 130) & (H < 170)
                    # if more than 15% of body is purple, it's likely the violet good fruit
