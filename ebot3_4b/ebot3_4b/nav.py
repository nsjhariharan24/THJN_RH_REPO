...
class LidarReactiveNavigator(Node):
    ...
    def start_turn(self):
        """CHANGED: Now rotates LEFT 90° when obstacle detected"""
        self.stop_robot()
        # LEFT turn: +90° (counter-clockwise)
        self.target_yaw = normalize_angle(self.current_yaw + math.pi / 2)
        self.state = "TURNING"
        self.get_logger().info("Obstacle detected → ROTATING LEFT 90°")
    ...
    def handle_turn(self):
        ...
        if abs(error) > 0.03:
            twist = Twist()
            # LEFT turn: positive angular velocity
            ...
            self.get_logger().info(
                f"90° LEFT turn completed → count={self.lane_turn_count}"
            )
