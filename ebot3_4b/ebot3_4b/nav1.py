...
class SquareDetectorForceStop(Node):
    ...
    def scan_cb(self, msg: LaserScan):
        ...
        deg = math.degrees(self.robot_yaw)
        cond = (-8 < deg < 8) or (172 < abs(deg) < 188)
        if not cond:
        ...
        if self.square_detect_time is not None and (now - self.square_detect_time >= self.delay_seconds):
            if self.forced_stop_timer is None:
                ...
                self.forced_stop_timer = self.create_timer(interval, self._publish_zero_cmd_cb)
    ...
    def _publish_zero_cmd_and_check_end(self):
        now = self.get_clock().now().seconds_nanoseconds()[0]
        self._publish_zero_cmd()
        if now >= self.forced_stop_end_time:
            try:
                msgf = Bool(); msgf.data = False; self.shape_pub.publish(msgf)
                if self.last_status != "NO_SHAPE":
                    st = String(); st.data = "NO_SHAPE"; self.status_pub.publish(st); self.last_status = "NO_SHAPE"
                self.get_logger().info("🟢 Forced STOP ended: published False and resumed.")
            finally:
                if self.forced_stop_timer is not None:
                    try:
    ...
    def _publish_zero_cmd_cb(self):
        self._publish_zero_cmd_and_check_end()
...
class SimpleTriangleDetector(Node):
    def __init__(self, plant_mgr: PlantIDManager):
        ...
        # flags (handles BOTH triangle + dock)
        ...
        self.get_logger().info("🔺 SimpleTriangleDetector started (FLAG logic for BOTH triangle + dock).")
    ...
    def scan_cb(self, msg: LaserScan):
        deg = math.degrees(self.robot_yaw)
        cond = (-10 < deg < 10) or (170 < abs(deg) < 190)
        if not cond:
            self._publish_no_shape()
            return
        ...
        # ============================================================
        #     FLAG LOGIC WORKS FOR BOTH TRIANGLE + DOCK (SAME)
        # ============================================================
        now = self.get_clock().now().seconds_nanoseconds()[0]

        if candidate is not None:
            corner = candidate['corner']
            rx = 0.0; ry = float(corner[1])
            gx = float(self.robot_x)
            gy = self.robot_y + ry

            # ------- FLAG DEDUP LOGIC (SAME FOR TRIANGLE + DOCK) -------
            too_close = False
            ...
            if not too_close:
                ...
                # Log what type of flag (triangle or dock)
                is_dock_zone = (self.dock_x_min <= gx <= self.dock_x_max) and (gy < self.dock_y_max)
                flag_type = "⚓ DOCK" if is_dock_zone else "🔺 TRIANGLE"
                self.get_logger().warn(f"{flag_type} FLAG added @ ({gx:.3f},{gy:.3f})")

                # Check for 2nd flag confirmation
                if len(self.flags) >= 2:
        ...
        # ----------- confirm stop (3.2s delay) -----------
        if (self.confirm_time is not None) and (now - self.confirm_time >= self.immediate_confirm_delay):
            if self.forced_stop_timer is None and (now - self.last_stop_publish_time >= self.post_publish_cooldown):
    ...
    # ====================================================================
    #                STOP HANDLER (DETECTS DOCK vs TRIANGLE)
    # ====================================================================
    def _trigger_forced_stop_immediate(self):
        ...
        # CHECK LAST 2 FLAGS: DOCK vs TRIANGLE
        if len(self.flags) >= 2:
            f1x, f1y = self.flags[-2]
            f2x, f2y = self.flags[-1]
            
            # BOTH flags in dock zone → DOCK_STATION
            if (self.dock_x_min <= f1x <= self.dock_x_max and f1y < self.dock_y_max) and \
               (self.dock_x_min <= f2x <= self.dock_x_max and f2y < self.dock_y_max):
                st = String()
                st.data = f"DOCK_STATION,{gx_report:.3f},{gy_report:.3f},0"
                self.get_logger().warn(f"⚓ DOCK_STATION CONFIRMED (2 dock flags)")
            else:
                # Normal triangle → plant ID
                ...
                self.get_logger().warn(f"🔺 FERTILIZER_REQUIRED CONFIRMED (plant {plant_id})")
        else:
...
def main():
    ...
    try:
        ...
        try:
            while rclpy.ok():
                # always spin briefly so odom updates

                current_yaw = square_node.robot_yaw
                deg = math.degrees(current_yaw)

                #print(f"Yaw (rad): {current_yaw:.4f} | Yaw (deg): {deg:.2f}")
                cond=True

                # allow heavy processing only at ±90°
                if cond:
                    # your detection logic already lives in callbacks
                    executor.spin_once(timeout_sec=0.1)
                else:
                    # skip heavy processing, just update odom
                    if (last_yaw is None or abs(current_yaw - last_yaw) > YAW_EPS):
                        executor.spin_once(timeout_sec=0.1)
                    else:
